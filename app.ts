...
class CSVAnalyzer {
    ...
    private uniqueNotes: HTMLElement;
    private notesList: HTMLElement;
    private driverForm: HTMLFormElement;
    private driverResult: HTMLElement;
    private currentTotalRecords: number = 0;
    private currentRows: CSVRow[] = [];
    ...
    constructor() {
        ...
        this.uniqueNotes = document.getElementById('uniqueNotes') as HTMLElement;
        this.notesList = document.getElementById('notesList') as HTMLElement;
        this.driverForm = document.getElementById('driverForm') as HTMLFormElement;
        this.driverResult = document.getElementById('driverResult') as HTMLElement;
    ...
    private parseAndAnalyzeCSV(csvContent: string, file: File): void {
        const lines = csvContent.split('\n');
        const headers = this.parseCSVLine(lines[0]);
        
        // Find the index of the Note column
        const noteColumnIndex = headers.findIndex(header => 
        ...
        const noteFrequencies: NoteFrequency[] = Array.from(noteFrequencyMap.entries())
    ...
    private displayResults(file: File, rows: CSVRow[], noteFrequencies: NoteFrequency[]): void {
    ...
    private handleDriverFormSubmit(event: Event): void {
        event.preventDefault();
        
        const formData = new FormData(this.driverForm);
        const originalDriver = formData.get('originalDriver') as string;
        const helperName = formData.get('helperName') as string;
        const papersToTake = parseInt(formData.get('papersToTake') as string);
    ...
    private calculateHelperNewspapers(startStopNumber: number): NoteFrequency[] {
        ...
        const helperNoteMap = new Map<string, number>();
    ...
    private displayDriverResult(originalDriver: string, helperName: string, papersToTake: number, stopNumber: number, helperNewspapers: NoteFrequency[]): void {
